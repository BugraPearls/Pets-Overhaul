...
namespace PetsOverhaul.PetEffects
{
    public sealed class MiniMinotaur : PetEffect
    {
        ...
        {
            if (PetIsEquipped() && PetUtils.ValidTargetCheck(target) && Pet.timer <= 0 && item.CountsAsClass<MeleeDamageClass>())
            {
                if (minotaurStack < 80)
                {
                    minotaurStack += 2;
        ...
        {
            if (PetIsEquipped() && PetUtils.ValidTargetCheck(target) && Pet.timer <= 0 && proj.CountsAsClass<MeleeDamageClass>())
            {
                if (minotaurStack < 80)
                {
                    minotaurStack += 1;

    ...
    public sealed class Lizard : PetEffect
    {
        ...
        public override void ExtraProcessTriggers(TriggersSet triggersSet)
        {
            if (Pet.AbilityPressCheck() && PetIsEquipped())
            {
                ...
                NPC.NewNPC(PetUtils.GetSource_Pet(EntitySourcePetIDs.PetNPC), (int)Player.Center.X, (int)Player.Center.Y, ModContent.NPCType<LizardTail>(), ai0: Player.statLifeMax2 * tailMaxHp, ai1: (int)(tailWait * (1 / (1 + Pet.abilityHaste))), ai2: Pet.timerMax / 2);

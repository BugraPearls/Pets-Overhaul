...
namespace PetsOverhaul.PetEffects
{
    public sealed class IceQueen : PetEffect
    {
        ...
        public override void DrawEffects(PlayerDrawSet drawInfo, ref float r, ref float g, ref float b, ref float a, ref bool fullBright)
        {
            ...
            {
                PetUtils.CircularDustEffect(Player.Center, DustID.SnowflakeIce, queenRange);
            }
        }
        ...
        public override void PostUpdateMiscEffects()
        {
            if (PetIsEquipped() && frozenTomb == true)
            {
                foreach (var npc in Main.ActiveNPCs)
                {
                    ...
                    {
                        PetGlobalNPC.AddSlow(new PetSlow(slowAmount, 1, PetSlowID.IceQueen), npc, Player);
                    }
                }
                ...
                if (iceQueenFrame >= tombTime)
                {
                    foreach (var npc in Main.ActiveNPCs)
                    {
                        ...
                        {
                            Pet.PetStrike(npc, freezeDamage, 2, Main.rand.NextBool((int)Math.Min(Player.GetTotalCritChance<GenericDamageClass>(), 100)), 0, DamageClass.Generic);
                        }
                    }
